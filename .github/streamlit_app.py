...
if mostrar_flujo_caja_escenarios:
    ...
    escenario_flujo = st.selectbox(
        "Seleccionar escenario",
        ["Pesimista", "Normal", "Optimista"],
        index=1,
        key="cash_flow_scenario_selector"
    )

    if escenario_flujo == "Pesimista":
        df_f = df_f_pes_num
    elif escenario_flujo == "Optimista":
        df_f = df_f_opt_num
    else:
        df_f = df_f_nor_num

    etiquetas = df_f["Fecha"].astype(str).tolist()
    valores = df_f["Flujo Neto"].tolist()
    acumulados = df_f["Flujo Acumulado"].tolist()

    import plotly.graph_objects as go
    fig_mix = go.Figure()

    # --- Barras Waterfall ---
    fig_mix.add_trace(go.Waterfall(
        name="Flujo Neto",
        orientation="v",
        x=etiquetas,
        y=valores,
        text=[f"{simbolo_moneda}{v:,.0f}" for v in valores],
        textposition="outside",
        connector={"line":{"color":"#004080"}},
    ...
    fig_mix.update_layout(
        title=f"Flujo de Caja – Escenario {escenario_flujo}",
        xaxis_title="Periodo",
        yaxis_title=f"Flujo Neto ({simbolo_moneda})",
    ...
    # --- Tablas debajo ---
    st.markdown("### 📑 Tablas de Flujo de Caja por Escenario")
...
st.markdown("### 📊 Comparativo de Flujo Acumulado – Escenarios")

fig_comp = go.Figure()

fig_comp.add_trace(go.Scatter(
    name="Pesimista",
    x=df_f_pes_num["Fecha"].astype(str),
    y=df_f_pes_num["Flujo Acumulado"],
    mode="lines+markers",
...
fig_comp.add_trace(go.Scatter(
    name="Normal",
    x=df_f_nor_num["Fecha"].astype(str),
    y=df_f_nor_num["Flujo Acumulado"],
    mode="lines+markers",
...
fig_comp.add_trace(go.Scatter(
    name="Optimista",
    x=df_f_opt_num["Fecha"].astype(str),
    y=df_f_opt_num["Flujo Acumulado"],
    mode="lines+markers",
    line=dict(color="#3498db", width=2)
))

fig_comp.update_layout(
    title="Comparativo de Flujo Acumulado por Escenario",
    xaxis_title="Periodo",
    yaxis_title=f"Flujo Acumulado ({simbolo_moneda})",
    template="plotly_white",
